...
func setup(c *caddy.Controller) error {
	...
	dnsserver.GetConfig(c).AddPlugin(func(next plugin.Handler) plugin.Handler {
		return Zlogger{Next: next, Rules: rules, repl: replacer.New(), logger: defaultLog()}
	})
...
func defaultLog() *zap.Logger {
	console := zapcore.NewConsoleEncoder(zapcore.EncoderConfig{MessageKey: "M"})
	ws := zapcore.AddSync(os.Stdout)
	lvl := zapcore.DebugLevel
	return zap.New(zapcore.NewCore(console, ws, lvl))
}
